...
The same rule governs the one-shot nudge. When a tool-calling node answers with no `tool_call` and no tool has run yet for that execution, the executor spends one extra turn asking for the call. That condition is read off the loop's structure — tool calling on, no `tool_call`, `state["tool_result"]` still empty — never off the wording of the reply, because a narration, a refusal and a confident guess are indistinguishable as text and impossible to enumerate across languages.
