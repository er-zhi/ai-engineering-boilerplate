...
- **A composed argument is checked before it is dispatched.** Composing is a generative act and its output is unchecked input: asked "weather there?", a model composed the place `"there"`, which is a real village, and the guard over the reply passed its weather at 0.96 because the reply was faithful to material faithful to a question nobody asked. One typed decision asks whether the message points at the composed value — by name, by an unambiguous description, or normalised from either. It does not, the turn asks for the value instead of inventing one, worded from the tool's own schema.
- **A follow-up carries what the earlier turn found.** Engine copies the parent execution's records — the call and what it returned, together — under a key of their own, so the scope decision still runs and still decides. That decision gains one option when there is such material: answer from it. The answer is then checked against it like any other.
- **A reply is checked against what came back before the person sees it.** One typed decision over (material, reply) asks two things: whether the reply merely undertakes work, and whether every specific value in it is present in the material. A fault is stated back — quoting the refused words, because a light model told only that it was wrong writes the same thing again — and asked once more; a second fault is answered plainly. The guards may only catch a fault, never invent one: an unreachable decider leaves the reply exactly as it was.
- What a source covers, and which sources are in circulation at all, are operator data — `declarative-tools.json` for whole rows, `built-in-tools.json` for the tools compiled in. No subject is named in Rust or in a prompt; every subject reaches a decision from the catalog at runtime.
